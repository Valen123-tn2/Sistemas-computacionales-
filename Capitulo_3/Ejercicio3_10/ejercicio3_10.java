package Ejercicio3_10;
import javax.swing.*;

public class ejercicio3_10 extends JFrame {
    
    public ejercicio3_10() {
        setTitle("Ejercicios 3.10");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        JTextField n = new JTextField(10);
        JButton submitButton = new JButton("Enviar");
        JLabel resultLabel = new JLabel("");

        submitButton.addActionListener(e -> {
            try {
                int num = Integer.parseInt(n.getText());
                while (1<= num && num <= 10){
                    resultLabel.setText("");
                    for (int i = 1; i <= 10; i++){
                      resultLabel.setText(resultLabel.getText() + num+" x "+ i + " = "+ num*i+ " | "); 
                    } 
                    break;
                }
                if (!(1 <= num && num <= 10)){
                    resultLabel.setText("Ingrese un Numero 1 al 10"); 
                }
            } catch (NumberFormatException ex) {
                resultLabel.setText("Entrada inválida. Introduce un número");
            }
        });

        JPanel panel = new JPanel();
        panel.add(new JLabel("Ingrese un Numero: "));
        panel.add(n);
        panel.add(submitButton);
        panel.add(resultLabel);

        add(panel);
        setSize(700, 200);
        setLocationRelativeTo(null);
    }
    public static void main(String[] args) {

        SwingUtilities.invokeLater(() -> {

            ejercicio3_10 ejercicio = new ejercicio3_10();
            ejercicio.setVisible(true);
        });
    }
}
